...
function formatDataset(dataset) {
  const termOrder = TERM_DEFS.map((d) => d.term)
  const lines = []
  ...
  lines.push(`export type SolarTermName = '${termOrder.join("' | '")}'`)
  lines.push(`export const SOLAR_TERM_ORDER: SolarTermName[] = ['${termOrder.join("','")}']`)
  lines.push('export const SOLAR_TERMS: Record<number, string[]> = {')

  const years = Object.keys(dataset).map(Number).sort((a, b) => a - b)
  for (const year of years) {
    const isos = dataset[year].map((entry) => entry.iso)
    lines.push(`  ${year}: ['${isos.join("','")}'],`)
  }
  lines.push('}')
  lines.push('')
  return lines.join('\n')
